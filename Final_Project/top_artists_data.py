...
#kris_top_artist_data = spotify.top_artists(100, 0, 'long_term')
...
musixmatch = Musixmatch('15e9c86958af4bf01cd2ec4e0a03818a')
#'f85963e4bac8c2c33b149cab49c2b81d'
...
class Database:
    ...
    def __init__(self, db_name):
        ...
        self.cur.execute("DROP TABLE IF EXISTS Top_Artists")
        self.cur.execute("CREATE TABLE Top_Artists (artist_id PRIMARY KEY, name TEXT)")
...
kris_top_artists_data = []

for artist in artists_dict['items']:
    artist_name = artist['name']
    artist_id = artist['id']
    kris_top_artists_data.append((artist_id, artist_name))
...
for artist in kris_top_artists_data: 
    artist_id = artist[0]
    name = artist[1]

    if name not in db_artists_list:
        db.cur.execute("INSERT INTO Top_Artists (artist_id, name) VALUES (?, ?)", (artist_id, name))
